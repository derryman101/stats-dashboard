    ...
    stages {
        ...
        stage('Deploy Locally') {
            steps {
                ...
                sh '''
                    docker stop stats-dashboard || true
                    docker rm stats-dashboard || true
                    docker run -d --name stats-dashboard -p 80:80 ${ECR_REGISTRY}.dkr.ecr.${AWS_REGION}.amazonaws.com/${IMAGE_NAME}:${IMAGE_TAG}
                '''
            }
        }
    }
